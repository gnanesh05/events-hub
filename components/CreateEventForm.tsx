...
import { createEvent, CreateEventState } from '@/lib/actions/event.actions';
...
interface Props {
  ...
  serverAction?: (prevState: CreateEventState, formData: FormData) => Promise<CreateEventState>;
}

const CreateEventForm = ({ mode = 'create', defaultValues, serverAction }: Props) => {
  const isEdit = mode === 'edit';
  const actionToUse = serverAction ?? createEvent;

  const transformedAction = async (prevState: CreateEventState, formData: FormData) => {
    ...
    return actionToUse(prevState, formData);
  };
  ...
  useEffect(() => {
    if (state.success && !successHandled.current) {
      ...
      if (isEdit) {
        ...
        posthog.capture('event_created', { data: state.data });
        router.push('/');
      }
    }
  ...
  }, [state.success, router, state.data, state.errors, isEdit]);

  const val = <K extends keyof EventDefaultValues>(field: K): EventDefaultValues[K] | string => {
    return (state.data as Record<string, unknown>)?.[field as string] as EventDefaultValues[K] ?? defaultValues?.[field] ?? '';
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] py-8">
      <div className="w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <form className="space-y-8" action={formAction}>
          {isEdit && (
            <input type="hidden" name="existingImage" value={defaultValues?.image ?? ''} />
          )}
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-dark-200/30 backdrop-blur-md rounded-xl p-6 space-y-6 border border-primary/20 shadow-lg shadow-primary/5 transition-all duration-300 hover:border-primary/30 hover:shadow-primary/10">
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="overview" className="text-sm font-medium text-foreground/80">Overview</label>
                  <textarea
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="image" className="text-sm font-medium text-foreground/80">
                    {isEdit ? 'New Image (leave empty to keep existing)' : 'Image'}
                  </label>
                  {isEdit && defaultValues?.image && (
                    <img src={defaultValues.image} alt="Current event" className="h-24 w-auto rounded-lg object-cover mb-2" />
                  )}
                  <input
              ...
              <div className="bg-dark-200/30 backdrop-blur-md rounded-xl p-6 space-y-6 border border-primary/20 shadow-lg shadow-primary/5 transition-all duration-300 hover:border-primary/30 hover:shadow-primary/10">
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="agenda" className="text-sm font-medium text-foreground/80">Agenda (one item per line)</label>
                  <textarea
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="tags" className="text-sm font-medium text-foreground/80">Tags (comma-separated)</label>
                  <input
            ...
            <div className="space-y-6">
              ...
              <div className="bg-dark-200/30 backdrop-blur-md rounded-xl p-6 space-y-6 border border-primary/20 shadow-lg shadow-primary/5 transition-all duration-300 hover:border-primary/30 hover:shadow-primary/10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <label htmlFor="time" className="text-sm font-medium text-foreground/80">Time</label>
                    <input
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="mode" className="text-sm font-medium text-foreground/80">Mode</label>
                  <select
                  ...
                  {state.errors?.mode && <p className="text-sm text-red-500">{state.errors.mode[0]}</p>}
                </div>

                <div className="flex flex-col gap-2">
                  <label htmlFor="venue" className="text-sm font-medium text-foreground/80">Venue</label>
                  <input
              ...
              <div className="bg-dark-200/30 backdrop-blur-md rounded-xl p-6 space-y-6 border border-primary/20 shadow-lg shadow-primary/5 transition-all duration-300 hover:border-primary/30 hover:shadow-primary/10">
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="organizer" className="text-sm font-medium text-foreground/80">Organizer</label>
                  <input
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="audience" className="text-sm font-medium text-foreground/80">Audience</label>
                  <input
              ...
              <div className="bg-dark-200/30 backdrop-blur-md rounded-xl p-6 space-y-6 border border-primary/20 shadow-lg shadow-primary/5 transition-all duration-300 hover:border-primary/30 hover:shadow-primary/10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label htmlFor="bookingSlots" className="text-sm font-medium text-foreground/80">Booking Slots</label>
                    <input
